...
/**
 * 
 * @On(event = { "Action1" }, entity = "bTPGenAI4S4_StudentAmer47_ex2Srv.CustomerMessage")
...
try {
    ...
    customerMessage = await SELECT.one.from('BTPGenAI4S4_StudentAmer47_ex2.CustomerMessage').where({ ID });
    if (!customerMessage) {
...
try {
    ...
    await UPDATE('BTPGenAI4S4_StudentAmer47_ex2.CustomerMessage').set({
