...
/**
 * 
 * @After(event = { "UPDATE","CREATE" }, entity = "bTPGenAI4S4_StudentAmer47_ex2Srv.ProductFAQ")
...
try {
    ...
    productFAQ = await SELECT.one('BTPGenAI4S4_StudentAmer47_ex2.ProductFAQ').where({ ID: productFAQID }).forUpdate();
    if (!productFAQ) {
...
try {
    ...
    await UPDATE('BTPGenAI4S4_StudentAmer47_ex2.ProductFAQ').set({ embedding: embedding }).where({ ID });
